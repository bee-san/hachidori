// Drives the test relay server (extension/sharing-relay.js on a plain Node
// WebSocket server, standing in for GameSentenceMiner) over raw sockets.
...
async function server(t) {
  const started = await startSharingRelayServer({ pingMs: 50 });
...
test("clients are refused until a host connects, and a second host is turned away", async (t) => {
  ...
  assert.deepEqual(await second.json(), { kind: "listen-failed", error: "Another Hachidori is already sharing through GameSentenceMiner." });
...
test("losing the host closes its clients, and the next host is accepted", async (t) => {
  const { port, relay } = await server(t);
  const host = await connectClient(t, port, "/host");
  await host.json();
  const client = await connectClient(t, port, "/link");
  ...
  assert.deepEqual(await next.json(), { kind: "listening", port });
});
