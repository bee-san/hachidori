...
// fixtures are the engine's own, generated by tests/fixtures/mdict/gen_fixtures.py:
// an HTML MDX with an @@@LINK alias, duplicate headwords, a StyleSheet
// substitution and an MDD holding a PNG, a CSS file and a traversal key.
// hdw_import's title pre-check reads index.json out of a ZIP, so this is also
// the proof that an MDict file gets past it and through the same staging.
const MDX_FIXTURES = join(HERE, '..', 'third_party', 'hoshidicts', 'tests', 'fixtures', 'mdict');
