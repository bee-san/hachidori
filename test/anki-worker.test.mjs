...
function fixture(firstAudio = false) {
  ...
  const service = createAnkiWorkerService({ gateway, readOptions: async () => options,
    ...
    engine: async message => { calls.push(message.type); return { generation }; },
    offscreen: async message => {
      calls.push(message.type);
      if (message.type === "hd_anki_audio") return { filename: "checked.wav", data: "YXVkaW8=" };
  ...
  return { service, calls, request, changedGeneration() { generation++; }, get fields() { return fields; } };
}
