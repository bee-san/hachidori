...
| Anki | Requests go to `http://127.0.0.1:8765`. After Start setup, Anki discovery reads deck/model/card/note metadata. A scoped duplicate index refreshes every 30 minutes and stores only each word, its aggregate maturity flag and matching note IDs; mining misses can perform a scoped repair lookup. Mature-word blur reads that local index. Explicit mining can send selected text, definitions, page title, audio, images and captured media according to field mappings. Anki controls any subsequent sync. | [Gateway](../extension/anki.js), [setup](../extension/anki-setup.js), [index](../extension/anki-index.js), [mining](../extension/anki-mining.js) |
