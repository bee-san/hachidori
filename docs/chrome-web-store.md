...
- **Continuous capture:** it is off by default, requires Start capture and Chrome's source
  ...
  linked browser, final JPEG/AVIF/WAV assets go through the sharing host, which
  owns the AnkiConnect transaction.
...
| Continuous capture | User-selected tab/window/monitor frames and available source audio stay in transient capture history after Start capture and Chrome's picker. Stop clears it; explicitly mined final clips are sent to the configured Anki. When linked, only final AVIF/WAV assets cross the relay to the host. Reading-page titles/URLs identify the linked source. | [Capture privacy](media-capture.md#privacy-and-limitations) |
