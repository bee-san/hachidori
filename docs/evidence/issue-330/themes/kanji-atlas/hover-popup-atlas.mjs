...
// Theme A/B copy of hover-popup.mjs (issue #334 evidence, not committed): identical
// settings except popupTheme, plus a probe timing every runThemeHook call.
const settings = { hoverEnabled: true, lookupMode: 'hover', hoverDelayMs: 0, popupNestingMaxDepth: 2,
  popupWidthPx: 520, popupHeightPx: 500, popupColumns: 1, maxResults: 32, popupToolbarPosition: 'top',
  popupTheme: process.env.HACHIDORI_BENCH_THEME ?? 'default',
  definitionBlurEnabled: false, showCompactDefinitionSummary: true, compactDefinitionSummaryCount: 3 };
const hookProbe = `(() => { const original = runThemeHook; globalThis.__hookTimes = [];
  runThemeHook = (level, kind) => { const t0 = performance.now(); original(level, kind);
    globalThis.__hookTimes.push({ kind, depth: level.depth, ms: performance.now() - t0, entries: level.popup?.querySelectorAll('.gsm-hoshidicts-entry, .gsm-hoshidicts-kanji-entry').length ?? 0 }); }; })();`;
...
try {
  for (let session = 0; session < samples; session++) {
    ...
    try {
      ...
      const hookTimes = JSON.parse(await evaluate('JSON.stringify(globalThis.__hookTimes || [])'));
      writeFileSync(resolve(output, `session-${session}-hook-times.json`), JSON.stringify({ theme: settings.popupTheme, hookTimes }, null, 2));
      console.log(JSON.stringify({ session, theme: settings.popupTheme, hookCalls: hookTimes.length }));
    } finally {
      await browser?.close();
